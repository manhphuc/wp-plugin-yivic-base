...
- We found a way to make Yivic Base load only once even when there's a Yivic Base plugin activated and many other plugins, themes use Yivic Base as a composer package. We need to keep a very good compability when we update the plugin Yivic Base later.
